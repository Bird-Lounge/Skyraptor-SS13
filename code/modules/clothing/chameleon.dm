...
/datum/action/item_action/chameleon/change/proc/update_look(mob/user, obj/item/picked_item)
	...
	if(isliving(user))
		...
		if(isitem(target)) //NK006 EDIT BEGIN: this is a little ugly but it should bring chameleons into functionality with alt-bodytype greyscales
			...
			if(initial(picked_item.greyscale_config_worn_bodytypes) && initial(picked_item.greyscale_colors))
				if(ishuman(user))
					...
					if(altbody)
						item_target.worn_icon = SSgreyscale.GetColoredIconByType(initial(altbody), initial(picked_item.greyscale_colors)) //NK006 EDIT END
