	...
	if((last_shot + fire_delay) <= world.time)
		...
		switch(strength)
			if(0)
				...
				playsound(src, 'sound/machines/cyclotron.ogg', 30, FALSE)
			if(1)
				P = new/obj/effect/accelerated_particle(T)
				playsound(src, 'sound/machines/cyclotron.ogg', 70, FALSE)
			if(2)
				P = new/obj/effect/accelerated_particle/strong(T)
				playsound(src, 'sound/machines/cyclotron.ogg', 100, FALSE)
			if(3)
